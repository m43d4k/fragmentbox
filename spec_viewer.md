...
- タグ（#tag形式）はカード上でハイライト表示、クリックでタグ絞り込み
  - タグ名は英数字・アンダースコアに加え日本語等のUnicode文字も対象（JS側は \p{L}\p{N} で判定）
- URLはリンクプレビューカード形式で表示（サイト名 / タイトルリンク / 説明文）
  - タイトル1行、説明文2行でclamp
- 本文中の連続する空行は1行分に圧縮して表示（ファイルの改行数に依存しない）
- 画像（Markdown画像記法）はサムネイル表示（最大240×180px）。クリックでライトボックス（全画面拡大）を開く
  - ライトボックスはESC / クリックで閉じる
  - 画像ファイルが存在しない場合は「(画像なし)」プレースホルダーを表示
...
- ヘッダー右端の♥ボタンでお気に入りのみ表示に切り替え（再クリックで解除）

編集（inbox / archive 両方）:
- カードフッターにペンシルアイコンボタンを表示
- ボタン押下で画面左上に編集モーダルを表示（オーバーレイ付き）
- モーダル内の textarea にフラグメントの生テキストを表示・編集可能
- Save ボタンでサーバーへ PUT、成功後にカード表示を即時更新
- Cancel / ESC / オーバーレイクリックでキャンセル（変更は破棄）
...
- PUT /api/fragments/{id}?source=inbox|archive — フラグメント本文を上書き保存、更新後のフラグメントを返す
